//*** Day 5 Exercise 1 ***
var naughtyOrNice1 = function(stringList) {
...
//*** Day 5 Exercise 2 ***
var naughtyOrNice2 = function(stringList) {
  var stringArray = stringList.split('|');
  var niceCount = 0;
  var skipMe = false;
  ...
  stringArray.forEach(function(string, idx) {
    ...
    for (i = 1; i < string.length; i += 1) {
      var cur = string.charAt(i);
      var pre1 = string.charAt(i - 1);
      var pre2 = string.charAt(i - 2);

      if (cur === pre2) {
        counts.repeatPlusMid += 1;
      }

      if (typeof counts.multiPairs[pre1 + cur] === 'undefined') {
        counts.multiPairs[pre1 + cur] = 0;
      }
      counts.multiPairs[pre1 + cur] += 1;
    ...
    if (counts.repeatPlusMid > 0) {
      for (var prop in counts.multiPairs) {
        if (counts.multiPairs[prop] > 1) {
          niceCount += 1;
          break;
        }
      }
...
//input = 'bzukgvyoqeiiii|iduapzchhhhfladn|fbpyzxdfmkrtfaeg|yzsmlbnftftgwadz'

console.log('Rules-1 Nice Strings:', naughtyOrNice1(input));
console.log('Rules-2 Nice Strings:', naughtyOrNice2(input));


//Probably should have done with regex matching. JS perf comparison:
//
var test = function(input) { 
    ...
    inputs.forEach(function(e) {
        ...
        if (hasPairs(e,pairs) && hasDoubles(e,e.split("")) ) {
          niceStrings++;
        }
    });
