//*** Day 6 Exercise 1 and 2 ***
function lightGrid(commands) {
  this.grid = {};
  this.commands = commands;
  this.totalBrightness = 0;
}

lightGrid.prototype.runCommands = function(interpreter) {
  this.commands.forEach(function(command) {
    var commandParsed = command.split(' ');
    if (commandParsed.length === 5) commandParsed.shift();
    
    var coord1 = commandParsed[1].split(',');
      coord1[0] = +coord1[0];
      coord1[1] = +coord1[1];
    var coord2 = commandParsed[3].split(',');
      coord2[0] = +coord2[0];
      coord2[1] = +coord2[1];

      if (interpreter === 'onoff') {
        this.alterPatchOfLights(coord1, coord2, commandParsed[0]);
      } else if (interpreter === 'updown'){
        this.alterPatchBrightness(coord1, coord2, commandParsed[0])
      }
  }.bind( this ));
  this.tallyBrightness();
};

lightGrid.prototype.alterPatchOfLights = function(corner1, corner2, commandType) {
  for (var i = corner1[0]; i <= corner2[0]; i += 1) {
    for (var j = corner1[1]; j <= corner2[1]; j += 1) {
      switch (commandType) {
        case 'toggle':
          this.toggleLight(i, j);
          break;
        case 'on':
          this.turnOnLight(i, j);
          break;
        case 'off':
          this.turnOffLight(i, j);
          break;
      }
    }
  }
...
lightGrid.prototype.toggleLight = function(x, y) {
  var coord = x.toString() + '-' + y.toString();
  if (typeof this.grid[coord] === 'undefined') {
    this.grid[coord] = true;
  } else {
    delete this.grid[coord];
  }
};

lightGrid.prototype.turnOnLight = function(x, y) {
  var coord = x.toString() + '-' + y.toString();
  if (typeof this.grid[coord] === 'undefined') {
    this.grid[coord] = true;
  }
...
lightGrid.prototype.turnOffLight = function(x, y) {
  var coord = x.toString() + '-' + y.toString();
  if (typeof this.grid[coord] !== 'undefined') {
    delete this.grid[coord];
  }
...
lightGrid.prototype.alterPatchBrightness = function(corner1, corner2, commandType) {
  for (var i = corner1[0]; i <= corner2[0]; i += 1) {
    for (var j = corner1[1]; j <= corner2[1]; j += 1) {
      switch (commandType) {
        case 'toggle':
          this.alterLightBrightness(i, j, 2);
          break;
        case 'on':
          this.alterLightBrightness(i, j, 1);
          break;
        case 'off':
          this.alterLightBrightness(i, j, -1);
          break;
...
lightGrid.prototype.alterLightBrightness = function(x, y, amount) {
  var coord = x.toString() + '-' + y.toString();
  if (typeof this.grid[coord] === 'undefined') {
    this.grid[coord] = amount;
  } else {
    this.grid[coord] += amount;
  }

  if (this.grid[coord] <= 0) {
    delete this.grid[coord];
  }
};

lightGrid.prototype.tallyBrightness = function() {
  console.log('caculating brightness')
  for (coord in this.grid) {
    this.totalBrightness += this.grid[coord];
  }
};
...
var myLights = new lightGrid(input.split('|'));
myLights.runCommands('onoff');
console.log(Object.keys(myLights.grid).length);

var myLights = new lightGrid(input.split('|'));
myLights.runCommands('updown');
console.log(myLights.totalBrightness);